...
\begin{enumerate}[leftmargin=*]
  ...
  \item \tool, a Kubernetes-native operator designed for read-only assessment in production clusters and implemented through declarative resources such as \crd{UpgradeAssessment}, \crd{UpgradeFinding}, \crd{UpgradeEvidence}, and \crd{UpgradePlan}.
...
The study should compare against Pluto, kubent/kube-no-trouble, a manual checklist executed by a platform engineer, \tool{} without \kagent, and \tool{} with \kagent. Baseline configuration, Kubernetes versions, manifests, and command invocations must be archived.
...
\bibitem{pluto}
Fairwinds, ``Pluto: A utility to help users find deprecated Kubernetes API versions in their code repositories and helm releases,'' \placeholder{REFERENCE\_TO\_BE\_COMPLETED}.

\bibitem{kubent}
Doit International, ``kube-no-trouble (kubent): Easily check your clusters for use of deprecated APIs,'' \placeholder{REFERENCE\_TO\_BE\_COMPLETED}.

\bibitem{kubernetes}
Kubernetes Documentation, ``Deprecated API migration guide,'' \placeholder{REFERENCE\_TO\_BE\_COMPLETED}.
\end{thebibliography}
