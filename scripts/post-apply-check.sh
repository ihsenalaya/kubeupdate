...
for app in "\${apps[@]}"; do
  kubectl -n argocd wait --for=jsonpath='{.status.health.status}'=Healthy "application/\${app}" --timeout=10m
done
...
not_ready_pods="\$(kubectl get pods -A --field-selector=status.phase!=Running,status.phase!=Succeeded --no-headers 2>/dev/null | awk '\$1 != "aks-command"' || true)"
